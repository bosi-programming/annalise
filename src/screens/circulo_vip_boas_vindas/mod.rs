...
pub fn CirculoVipBoasVindas() -> impl IntoView {
    ...
    view! {
        <main class="bg-[url(/starbg.gif)] bg-blend-color-burn bg-purple-900 lg:h-[calc(100dvh-81px)] lg:flex lg:flex-col lg:justify-center lg:gap-8 lg:px-26 h-full pt-10 px-8">
            <div class="lg:grid lg:grid-cols-2 lg:gap-10 lg:items-end max-sm:min-h-[calc(100dvh-65px)]">
                <Title text="Círculo VIP - Boas Vindas" />
                <div>
                    <Text r#as=ElementType::H1 size={title_size} weight=TextWeight::Bold class="text-white">
                        "Boas-vindas ao Círculo VIP"
                    </Text>
                </div>
                <Text r#as=ElementType::H2 size=TextSize::H2 class="text-white">
            ...
            <Button class="lg:w-65" variant=Variant::Secondary><a href="/circulo-vip">"Voltar para formulário"</a></Button>
        </main>
    }
}
